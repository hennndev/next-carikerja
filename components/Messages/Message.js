...
const Message = ({id, data, handleMessageTerm}) => {
    ...
    return (
        <div className="flex-between py-3 px-4 rounded-full cursor-pointer shadow-md relative hover:shadow-lg" onClick={handleClick}>
            {data.status === 'unread' && (
                <div className='w-3 h-3 bg-red-500 rounded-full absolute right-0 top-0'/>
            )}
            <div className="flex items-center space-x-1 md:space-x-3 mr-5 text-gray-700">
                <AiFillStar onClick={e => handleStar(e)} className={`cursor-pointer hover:text-orange-700 ${data.star && 'text-orange-700'}`}/>
                <h1 className="text-sm line-clamp-1">{data?.namaPerusahaan ? data?.namaPerusahaan : data?.nama}</h1>
            </div>
            <div className="flex-between flex-1 space-x-2 text-sm text-gray-500">
                <p className="line-clamp-1 text-sm">{data?.pesan}</p>
